...
"""
A simple automaton to model a scientific subject of interest
for a USV with autonomous scientist capability
"""
...
class NaschImagePlayer(cage.Player):
	...
	def __init__(self, width, height, numIterations):
	...
	def printImage (self):
		self.image.save ('iter_' + str (self.iteration) + '.png')
...
def exeEvents (events, gridMap):
	for event in events:
		...
		if (command == "seed"):
			...
			points = radMask ( (row, col), radius, np.random.random ( (gridMap.height, gridMap.width)))
			for point in points:
...
def initMapByImage (filename):
	...
	for row in range (im.size[0]):
		for col in range (im.size[1]):
			...
			if (p == (0, 106, 106)): # green       # RECALL: (0, 139, 0)
				grid[col][row] = 1

			elif (p == (129, 50, 234)): # purple  # RECALL: (224, 102, 255)
				grid[col][row] = 2

			elif (p == (65, 167, 234)): # blue     # RECALL: (65, 105, 225)
				grid[col][row] = 3
...
def main ():
	...
	player = NaschImagePlayer (gridWidth, gridHeight, numIterations)
